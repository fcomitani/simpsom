...
def _euclidean_squared_distance_part(x, w, w_flat_sq=None, xp=None):
...
def _euclidean_squared_distance(x, w, w_flat_sq=None, xp=None):
    """Calculate squared L2 distance
    NB: result shape is (N,X*Y)
    """

    x_sq = xp.power(x, 2).sum(axis=1, keepdims=True)
    return _euclidean_squared_distance_part(x, w, w_flat_sq, xp) + x_sq

def _euclidean_distance(x, w, w_flat_sq=None, xp=None):
    ...
    return xp.nan_to_num(
        xp.sqrt(
            _euclidean_squared_distance(x, w, w_flat_sq, xp)
        )
    )

def _cosine_distance(x, w, w_flat_sq=None, xp=None):
...
def _manhattan_distance(x, w, xp=None):
...
def batchpairdist(x, w, metric, sq=None, xp=None):
    ...
    if metric not in ["cosine", "euclidean", "manhattan"]:
        ...
        sys.exit()
    if metric=="cosine":
        return _cosine_distance(x, w, w_flat_sq=sq, xp=xp)
    elif metric=="euclidean":
        return _euclidean_distance(x, w, w_flat_sq=sq, xp=xp)
    else: # Slow
